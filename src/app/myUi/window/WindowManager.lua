...
local Z_ORDER = 1000
local logger = g.Logger.new("WindowManager")
...
function WindowManager:addWindow(window, isModal, isBgCoverTouchClose, noOpenAnim, background)
    logger:debugf("isModal = %s, isBgCoverTouchClose = %s, noOpenAnim = %s", isModal, isBgCoverTouchClose, noOpenAnim)
    ...
    logger:debugf("window:getLocalZOrder() = %s", window:getLocalZOrder())
    -- 更改模态的zOrder
    if isModal then
